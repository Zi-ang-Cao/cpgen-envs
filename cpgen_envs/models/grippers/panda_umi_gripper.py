...
class PandaUmiGripperBase(GripperModel):
    """
    Gripper for Franka's Panda (has two fingers).
    ...
    def __init__(self, idn=0, xml=None):
        if xml is None:
            xml = str(pathlib.Path(cpgen_envs.__file__).parent / "models/assets/grippers/panda_umi_gripper.xml")
        super().__init__(xml, idn=idn)
...
@register_gripper
class PandaUmiGripper(PandaUmiGripperBase):
    """
