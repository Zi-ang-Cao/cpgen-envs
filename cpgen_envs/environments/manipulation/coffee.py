...
class CoffeeWide(Coffee_D1):
    """
    Extends Coffee_D1 with random scaling of objects.
    """
    ...
    def _get_object_scale_bounds(self):
        ...
        return dict(
            coffee_pod=dict(
                scale_min=(0.6),  # 3D minimum scale bounds
                scale_max=(1.1),  # 3D maximum scale bounds
            ),
            coffee_machine=dict(
                scale_min=(0.6),  # 3D minimum scale bounds
                scale_max=(1.1),  # 3D maximum scale bounds
    ...
    def _load_model(self):
        ...
        # initialize objects of interest with random scales
        coffee_pod_bounds = self._get_object_scale_bounds()["coffee_pod"]
        coffee_machine_bounds = self._get_object_scale_bounds()["coffee_machine"]
        
        coffee_pod_scale = np.random.uniform(coffee_pod_bounds["scale_min"], coffee_pod_bounds["scale_max"], size=3)
        coffee_machine_scale = np.random.uniform(coffee_machine_bounds["scale_min"], coffee_machine_bounds["scale_max"], size=3)
        
        # set x-y scale to be the same
        coffee_pod_scale[0] = coffee_pod_scale[1] = coffee_machine_scale[0] = coffee_machine_scale[1]
        # set z scale to be the same
        coffee_pod_scale[2] = coffee_machine_scale[2]

        self.coffee_pod = CoffeeMachinePodObject(name="coffee_pod")
        self.coffee_machine = CoffeeMachineObject(name="coffee_machine")
        
        self.coffee_pod.set_scale(coffee_pod_scale)
        self.coffee_machine.set_scale(coffee_machine_scale)
