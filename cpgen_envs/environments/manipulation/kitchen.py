...
from cpgen_envs.environments.manipulation.single_arm_env import SingleArmEnv
...
def _pre_action(self, action, policy_step=False):
    SingleArmEnv._pre_action(self, action, policy_step=policy_step)
    
def _post_action(self, action):
    reward, done, info = SingleArmEnv._post_action(self, action)

    # Check if stove is turned on or not
    self._post_process()        
    return reward, done, info

KitchenEnv.__init__ = kitchen_env__init__
KitchenEnv._setup_observables = _setup_observables
KitchenEnv._create_obj_sensors = _create_obj_sensors
KitchenEnv._pre_action = _pre_action
KitchenEnv._post_action = _post_action
...
class StoveObjectNew(StoveObject):
    ...
    def __init__(
        ...
        MujocoXMLObject.__init__(self, pathlib.Path(cpgen_envs.__file__).parent / "models/assets/objects/stove.xml",
...
class ButtonObjectNew(ButtonObject):
    ...
    def __init__(self, name, friction=None, damping=None):
        MujocoXMLObject.__init__(self, pathlib.Path(cpgen_envs.__file__).parent / "models/assets/objects/button.xml",
                         name=name, joints=None, obj_type="all", duplicate_collision_geoms=True)
...
class KitchenWide(Kitchen_D1):
    """
    Extends Kitchen_D1 with random scaling of objects.
    """
    ...
    def _load_model(self):
        """
        Loads model and applies random scaling to objects
        """
        # Sample object scales
        scale_bounds = self._get_object_scale_bounds()
        
        # Sample all base sizes before creating objects
        bread_scale = np.random.uniform(scale_bounds["bread"]["scale_min"], 
                                        scale_bounds["bread"]["scale_max"])
        pot_scale = np.random.uniform(scale_bounds["pot"]["scale_min"],
                                    scale_bounds["pot"]["scale_max"]) 
        stove_scale = np.random.uniform(scale_bounds["stove"]["scale_min"],
                                        scale_bounds["stove"]["scale_max"])
        button_scale = np.random.uniform(scale_bounds["button"]["scale_min"], 
                                        scale_bounds["button"]["scale_max"])

        # Create base objects with standard sizes
        Kitchen_D1._load_model(self)

        # Apply scales to objects
        ingredient_size = np.array([0.015, 0.025, 0.02]) * bread_scale
        self.bread_ingredient.size = ingredient_size 
        
        self.pot_object.set_scale([pot_scale, pot_scale, pot_scale])
        self.stove_object_1.set_scale([stove_scale, stove_scale, stove_scale])
        self.button_object_1.set_scale([button_scale, button_scale, button_scale])
