...
class MugCleanup(SingleArmEnv_MG):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            # mount_types="default",  # commented out for robosuite==1.5.0
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            # incompatible with robosuite==1.5.0
...
class MugCleanupWide(MugCleanup_D1):
    """
    Extends MugCleanup_D1 with random scaling of objects.
    """
    ...
    def _load_model(self):
        ...
        object_bounds = self._get_object_scale_bounds()["object"]
        
        drawer_scale = np.random.uniform(drawer_bounds["scale_min"], drawer_bounds["scale_max"], size=3)
        object_scale = np.random.uniform(object_bounds["scale_min"], object_bounds["scale_max"], size=3)

        # Set x-y scale to be the same for both objects
        drawer_scale[0] = drawer_scale[1]
        object_scale[0] = object_scale[1]

        object_scale[2] = min(object_scale[2], drawer_scale[2])  # make sure object fits in drawer

        self.drawer.set_scale(drawer_scale)
        self.cleanup_object.set_scale(object_scale)
...
from cpgen_envs.environments.manipulation.hammer_cleanup import DrawerObjectReal
class MugCleanupReal(RealEnvMixin, MugCleanupWide):
    ...
    def _get_drawer_model(self):
        ...
        self.drawer = DrawerObjectReal(name="DrawerObject")
        obj_body = self.drawer
        for material in [redwood, ceramic, lightwood]:
            tex_element, mat_element, _, used = add_material(
                root=obj_body.worldbody,
    ...
    def _reset_internal(self):
        ...
        Difference w.r.t super class is hardcoding obj_pos_to_set[2] = 0.82
        """
        super()._reset_internal()
