...
class ThreadingWide(Threading_D1):
    """
    Extends Threading_D1 with random scaling of objects.
    """
    ...
    def _get_object_scale_bounds(self):
        ...
        return dict(
            needle=dict(
                scale_min=((0.8, 1, 0.8)),  # 3D minimum scale bounds
                scale_max=((1.2, 1, 1.2)),  # 3D maximum scale bounds
            ),
            tripod=dict(
                scale_min=((0.8, 0.9, 0.8)),  # 3D minimum scale bounds
                scale_max=((1.2, 1.1, 1.2)),  # 3D maximum scale bounds
            ),
        )
    ...
    def _load_model(self):
        ...
        needle_bounds = self._get_object_scale_bounds()["needle"]
        tripod_bounds = self._get_object_scale_bounds()["tripod"]
        needle_scale = np.random.uniform(needle_bounds["scale_min"], needle_bounds["scale_max"], size=3)

        # update tripod scale max to be needle scale * 1.1
        tripod_bounds["scale_min"] = needle_scale
        tripod_bounds_min = np.array(tripod_bounds["scale_min"]) * 1.1
        tripod_bounds_min[1] = needle_scale[1] * 0.9
        tripod_bounds_max = np.array(tripod_bounds["scale_max"]) * 1.1
        tripod_bounds_max[1] = needle_scale[1] * 0.9
        tripod_scale = np.random.uniform(tripod_bounds_min, tripod_bounds_max, size=3)

        self.needle.set_scale(needle_scale)
        self.tripod.set_scale(tripod_scale)
